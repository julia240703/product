...
$(document).ready(function() {
  ...
  // ===== [ADD] helper: tampilkan teks panduan kalau template = "Detail Produk"
  function updateTitleHelp(){
    const isDetail = $('#banner_template_id option:selected')
                      .text().trim().toLowerCase() === 'detail produk';
    $('#titleHelp').toggle(isDetail); // #titleHelp ada di bawah input Judul Banner
  }
  ...
  function initDataTable() {
    dataTable = $('#banner-table').DataTable({
      ...
      columns: [
        ...
        { data:'image_path', name:'image_path', className:"text-center",
          render:function(data,type,row){
            if(row.image_path){
              return `<img src="${row.image_path}" alt="Preview"
                      style="width:50px;height:50px;object-fit:cover;cursor:pointer"
                      class="rounded image-preview"
                      data-image="${row.image_path}" data-title="${row.title||''}" data-template="${row.template_name}">`;
            }
  ...
  function resetModal(modalId, formId) {
    ...
    $('#current-image').empty();
    // ===== [ADD] sembunyikan help saat reset
    $('#titleHelp').hide();
  }
  ...
  $(document).on('click', '.add-banner', function() {
    ...
    // ===== [ADD] periksa apakah perlu tampilkan teks panduan
    updateTitleHelp();
  });
  ...
  function updateBannerOrder(bannerId, newOrder) {
    $.ajax({
      ...
      method: 'POST',
      data: { id: bannerId, order: newOrder, _token: $('meta[name="csrf-token"]').attr('content') },
  ...
  $('#templateForm').on('submit', function(e) {
    ...
    const url = isEdit ? '{{ route('admin.template.edit') }}' : '{{ route('admin.template.store') }}';

    $.ajax({
      url, method:'POST', data:formData, processData:false, contentType:false,
      success:function(){
        $('#templateModal').modal('hide');
        showMessage(isEdit ? 'Template berhasil diperbarui!' : 'Template berhasil ditambahkan!');
        loadTemplates(); loadTemplateOptions(); isEdit = false;
      },
      error:function(xhr){
  ...
  $('#bannerForm').on('submit', function(e) {
    ...
    $.ajax({
      ...
      success:function(){
        ...
        showMessage(isEdit ? 'Banner berhasil diperbarui!' : 'Banner berhasil ditambahkan!');
        dataTable.ajax.reload(null, false);
        loadTemplates();
        isEdit = false;
      },
  ...
  $('#banner_image').on('change', function() {
    ...
    if (file) {
      ...
      if (fileSizeMB > 10) {
        alert(`Ukuran file terlalu besar! Maksimal 10MB. Ukuran file Anda: ${fileSizeMB.toFixed(2)}MB`);
        this.value = '';
        return false;
      }
    }
  ...
  // ===== [ADD] toggle help saat user ganti template di modal
