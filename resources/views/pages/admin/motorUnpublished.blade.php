      ...
      <form method="POST" id="editMotorForm" enctype="multipart/form-data">
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div class="mb-3">
              <label class="form-label">Link URL Aksesoris Motor <small class="text-muted">(opsional)</small></label>
              <input type="url" class="form-control" name="accessory_url" id="edit_accessory_url"
                     placeholder="https://contoh.com/produk/aksesoris-beat">
              <div class="form-text">Opsional. Maks 255 karakter.</div>
            </div>
...
$(document).ready(function () {
    ...
    columns: [
      ...
      { data: 'product', name: 'product', orderable: false, searchable: false },
      {
        ...
        render: function (data) {
          ...
          // embed URLs sebagai data-attr agar JS Edit selalu punya sumber kebenaran
          const motorUrlAttr = data.motor_url ? String(data.motor_url).replaceAll('"','&quot;') : '';
          const accUrlAttr   = data.accessory_url ? String(data.accessory_url).replaceAll('"','&quot;') : '';
          return `
            <div class="btn-group">
              <button class="btn btn-sm btn-primary me-1 editBtn"
                ...
                data-motor_url="${motorUrlAttr}"
                data-accessory_url="${accUrlAttr}"
  ...
  $('#add_category_id').on('change', function () {
    loadTypes($(this).val(), $('#add_type_id'));
  });

  // Edit
  $('#edit_category_id').on('change', function () {
    loadTypes($(this).val(), $('#edit_type_id'));
  });
  ...
  $(document).on('click', '.editBtn', function () {
    ...
    // === URLs (ambil dari JSON atau fallback ke data-attr tombol) ===
    const btn = $(this);
    $('#edit_motor_url').val(motorData.motor_url || btn.data('motor_url') || '');
    $('#edit_accessory_url').val(motorData.accessory_url || btn.data('accessory_url') || '');
    ...
    if (motorData.thumbnail) {
      ...
      $('#current-thumbnail').html(`
        ...
        <img src="${baseUrl}/${motorData.thumbnail}" style="width:120px;height:120px;object-fit:cover;border-radius:8px;border:1px solid #ddd;">
      `);
    } else {
    ...
    if (motorData.accessory_thumbnail) {
      ...
      $('#current-accessory-thumbnail').html(`
        ...
        <img src="${baseUrl}/${motorData.accessory_thumbnail}" style="width:120px;height:120px;object-fit:cover;border-radius:8px;border:1px solid #ddd;">
      `);
    } else {
    ...
    if (motorData.feature_thumbnail) {
      ...
      $('#current-feature-thumbnail').html(`
        ...
        <img src="${baseUrl}/${motorData.feature_thumbnail}" style="width:120px;height:120px;object-fit:cover;border-radius:8px;border:1px solid #ddd;">
      `);
    } else {
